...
def main(prgArgs,djDir):
    ...
    if prgArgs.table == 'ActCompoundSC':
        ...
        entrySQL = """
                Select sc.cmpbatch_id, sc.sum_assay_id,
                    ...
                    sc.mscore_ave
                From  dsummary.sum_cmpbatch_sc sc
                """
        ...
        if djSource is None:
            ...
            for e in tqdm(qryCmp, total= nEntries, desc=prgArgs.table):
                ...
                qrySQL = entrySQL + f" Where sc.cmpbatch_id = '{cid}' And sc.n_cmpbatches = 1 "
                ...
                for idx,row in df.iterrows():
                    ...
                    if djAssay and djCompound and djSource: 
                        ...
                        djObj = Activity_Compound_Inhibition.get(djCompound,djAssay,djSource)
                        if djObj is None:
                            NewEntry = True
                            djObj = Activity_Compound_Inhibition()
                        ...
                        if djCompound is None:
                            row["Error"] = f" Structure  {row['cmpbatch_id']} not Found"
                            logger.error(f" {row['cmpbatch_id']} Compound not Found")
                        OutDict.append(row)
