...
def main(prgArgs,djDir):
    ...
    # ----------------------------------------------------------------------------------------
    if 'ActStructDR' in prgArgs.table :
    ...
    # ----------------------------------------------------------------------------------------
    elif 'ActStructSC' in prgArgs.table :
        qryStr = Activity_Structure_Inhibition.objects.all()
        nEntries = qryStr.count()
        logger.info(f" [{prgArgs.table}] Structures: {nEntries}")

        OutNumbers = {'Processed':0, 'Empty':0, 'Failed':0, 'New':0, 'Uploaded':0,}
        qryStr = Activity_Structure_Inhibition.objects.all().iterator()
        for djObj in tqdm(qryStr, total=nEntries, desc='[ActStructSC]'):
            OutNumbers['Processed'] += 1
